...
void Session::doReadLine()
{
    boost::asio::async_read_until(m_socket, m_readBuffer, "\r\n",
        std::bind(&Session::onReadLine, shared_from_this(),
...
std::unique_ptr<Command> Session::makeCommand(const std::vector<std::string>& args)
{
    if (args.empty()) {
        return nullptr;
    }
...
{
    auto command = makeCommand(m_parsedArgs);
    if (!command) {
        m_writeBuffer = "-ERR unknown or malformed command\r\n";
    } else {
        m_writeBuffer = command->execute(m_server);
    }
